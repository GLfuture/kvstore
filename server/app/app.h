...
public:
    ...
    {
        R = Reactor(event_num,buffer_size);
        Init(port,backlog,thread_num,buffer_size);
    }
    int Init(uint16_t port,uint16_t backlog,uint8_t thread_num,uint32_t buffer_size);
    void Result_Add_Future(int clientfd,std::shared_future<string>& res);
    string Get_An_Finished_Result();
    string Decode(string buffer);
    string Work(string rbuffer);
    void Run(){
        R.Event_Loop();
    }
private:
...
private:
    ...
    Reactor R;       
    map<int16_t,queue<std::shared_future<string>>> result;



    map<string,string> string_store;//string存储
    map<string,vector<string>> array_store;//array存储
    map<string,list<string>> list_store;//list存储
    map<string,map<string,string>> rbtree_store;//rbtree存储
    map<string,unordered_set<string>> set_store;//set存储
